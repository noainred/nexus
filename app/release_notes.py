...
RELEASE_NOTES = [
    ...
    },
    {
        ...
        "date": "2026-06-20",
        "changes": [
            {"type": "added", "text": "네트워크 Ping 지연 차트 시간축에 범위별 구분선(그리드) 추가 — 1일=1시간, 7일=0.5일, 30일=7일, 90일=14일, 365일=1개월 간격으로 세로 점선과 눈금 라벨(시/월·일)을 시·일·월 경계에 맞춰 표시. 독립 공개 페이지(/ping)와 앱의 '네트워크 체크' 탭에 동일 적용(양끝 전체 타임스탬프 라벨은 눈금 라벨로 대체)"},
        ],
    },
    {
        "version": "1.7.45",
    ...
    },
    {
        ...
        "date": "2026-06-20",
        "changes": [
            {"type": "added", "text": "매니저 로그인 계정 관리(아이디/비밀번호) 추가 — 설정 → 계정·보안에서 다중 사용자 계정을 만들고 역할(관리자/조회전용)을 부여. 비밀번호는 PBKDF2-SHA256(솔트) 해시로 accounts.json(0600)에 저장(추가 의존성 없음). 기존 환경변수 관리자 비밀번호는 부트스트랩 관리자로 계속 병행 사용"},
            {"type": "added", "text": "역할 기반 권한 — 조회전용 계정은 읽기만 가능하고 모든 변경 API는 403으로 차단(본인 비밀번호 변경·로그아웃은 허용). 계정 관리(생성/역할변경/삭제)는 관리자만 가능. 감사 로그에 작업 수행 계정(user) 기록"},
            {"type": "changed", "text": "로그인 화면에 아이디 입력란 추가(비우면 기존 관리자 비밀번호로 로그인). 세션 쿠키는 계정 토큰(서버 시크릿 서명)·부트스트랩 토큰(비밀번호 서명) 모두 만료(12h) 적용"},
        ],
    },
    {
        "version": "1.7.40",
