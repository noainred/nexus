...
RELEASE_NOTES = [
    ...
    },
    {
        ...
        "date": "2026-08-10",
        "changes": [
            {"type": "added", "text": "엣지 노드 연결 테스트/재연결 기능 — 설정의 엣지 목록에서 노드별 [연결 테스트] 버튼과 [전체 엣지 연결 테스트] 버튼으로 끊긴 엣지를 즉시 재점검(자동 1회 재시도 포함, 일시적 끊김이면 그 자리에서 재연결 확인). 응답 지연시간(ms)을 버전 옆에 표시하고, 결과를 행 단위로 바로 갱신. 보안상 설정에 등록된 엣지 URL만 테스트 허용(임의 URL 조회 차단)"},
        ],
    },
    {
        "version": "1.7.53",
        "date": "2026-08-10",
        "changes": [
            {"type": "changed", "text": "[보안패치 2] 슬레이브 설정 동기화가 프록시 인증 비밀번호를 지우던 문제 수정 — Nexus는 조회(GET) 응답에서 비밀번호를 가리므로, 읽어온 비밀번호 없는 인증 객체를 그대로 되쓰면 슬레이브 프록시 인증이 빈 비밀번호로 덮어써졌음. 비밀번호가 없는 인증은 재적용하지 않고(복원 기능과 동일 규약) '프록시 비밀번호 재입력 필요' 경고를 응답에 포함"},
        ],
    },
    {
        "version": "1.7.52",
    ...
    },
    {
        ...
        "date": "2026-08-10",
        "changes": [
            {"type": "added", "text": "네트워크 Ping 차트 헤더에 평균값·최대값 표시 — 기존 '평소(중앙값)'에 더해 '평균'과 '최대'를 함께 표기(예: 평소(중앙값) 212 · 평균 220 · 최대 415 ms). 평균·최대는 다운샘플 이전의 원시 측정값 기준으로 정확히 계산하며, 앱의 '네트워크 체크' 탭과 독립 공개 페이지(/ping)에 동일 적용"},
        ],
    },
    {
        "version": "1.7.50",
        "date": "2026-08-09",
        "changes": [
            {"type": "changed", "text": "자동 업데이트 설정 저장 버그 일괄 수정 — ① 소스 종류만 바꾸고 URL을 비워 저장해도 400으로 전체 저장이 실패하지 않도록 소스·URL 쌍을 유지하고 나머지 설정만 반영 ② 빈 URL 저장이 환경변수 폴백 URL을 파일에 고착시키지 않음 ③ 새 URL과 clear_url 동시 전송은 모호하므로 400 ④ 상태 조회가 끝나기 전 저장 시 엣지 서버 목록이 빈 배열로 지워지던 사고 방지(생략=유지) ⑤ '저장된 소스 URL 지우기' 체크박스 추가 — 상태 조회 실패 중에도 확실하게 삭제 가능"},
            {"type": "changed", "text": "auto-update.sh 워처 수정 — 소스 미설정 경고를 파일 마커로 1회만 기록(운영 oneshot 타이머에서 5분마다 로그가 쌓이던 문제), --watch 모드에서 포탈로 지운 URL/토큰이 셸 변수에 남아 제거된 소스에서 계속 설치되던 문제를 환경값 기준 리셋으로 해결"},
            {"type": "changed", "text": "네트워크 Ping 페이지 차트 캐시 정합성 수정 — 범위 버튼 연속 클릭/자동 갱신 경합 시 뒤늦은 이전 응답을 폐기(요청 순서 가드), 조회 실패 시 캐시를 비워 필터 입력이 죽은 데이터로 에러 메시지를 덮지 않음, 시간축 눈금 간격을 화면 선택값이 아닌 데이터가 속한 응답의 기간 기준으로 계산(불일치 시 차트당 수천 개 눈금선 생성 방지), 필터 입력 디바운스(150ms) 추가"},
            {"type": "changed", "text": "헤더 상단 4px 여백 규칙을 style.css로 통합(/ping 인라인 중복 제거), token 필드 주석을 실제 동작(빈 값=유지, 삭제는 clear_token)과 일치하게 수정"},
        ],
    },
    {
        "version": "1.7.49",
