...
| **보안 점검** | 사이트별 **익명 접근 허용 / 기본 admin 계정 활성 / nx-admin 권한 계정 수**를 동시(병렬) 점검하고 서버별 위험도(양호/점검 필요/조회 불가)와 플릿 요약을 표시 |
| **노드 계정 관리** | 선택한 서버(들)에 **관리자(nx-admin) 유저 생성** 및 **사용자 비밀번호 변경**(다중 노드 일괄). 매니저 접속 계정의 비번을 바꾸면 저장 자격증명도 자동 동기화(잠김 방지) |
...
`보안 점검` 탭은 전 서버를 병렬로 훑어 보안 설정을 한 표로 보여줍니다.

| 항목 | 의미 / 권장 |
| --- | --- |
| **익명 접근** | `허용`이면 인증 없이 읽기가 가능 — 폐쇄망이라도 **차단** 권장 |
| **기본 admin 계정** | 기본 `admin` 계정이 `활성` — 전용 운영 계정을 만들고(노드 계정 관리) 기본 계정은 잠그는 것을 권장 |
| **관리자 권한 계정** | `nx-admin` 역할 보유 계정 수/목록 — **최소화** 권장 |
| **상태** | 양호 / ⚠ 점검 필요(위험 항목 존재) / 확인 불가(권한·연결) |

> 점검 결과 위험이 보이면 **노드 계정 관리**에서 전용 관리자 계정을 만들고
> 기본 계정 비밀번호를 바꾼 뒤, Nexus UI에서 익명 접근을 끄세요. 계정 읽기에는
> 그 노드 계정의 보안 읽기 권한이 필요합니다(부족하면 '확인 불가').
